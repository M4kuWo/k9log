...
//
// Reads the file via expo-file-system's File class rather than
// `fetch(uri).blob()` — fetching a local file:// (or content://) URI
// directly is unreliable in React Native and can silently resolve to a
// bogus tiny response instead of the actual file, which is exactly what
// happened before this fix (a 14-byte "File not found" text body got
// uploaded as if it were the photo).
